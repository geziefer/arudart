...
- [x] 1.1 Write property test for event JSON serialization
  - **Property 7: Event JSON Serialization Round Trip**
  - **Validates: Requirements AC-7.6.7**

- [ ] 2. Implement CoordinateFusion class
  - [x] 2.1 Create `src/fusion/coordinate_fusion.py` with CoordinateFusion class
  ...
  - [x] 2.2 Implement outlier rejection algorithm
  ...
  - [x] 2.3 Implement confidence-weighted average
...
- [ ] 3. Implement PolarConverter class
  - [x] 3.1 Create `src/fusion/polar_converter.py` with PolarConverter class
  ...
  - [x] 3.2 Implement `polar_to_cartesian()` method
    - Compute x = r × cos(θ)
    - Compute y = r × sin(θ)
    - _Requirements: AC-7.2.5_
  
  - [x] 3.3 Add helper methods for angle conversion
    - `radians_to_degrees()` and `degrees_to_radians()`
    - _Requirements: AC-7.2.2_

- [x] 3.4 Write property test for polar coordinate round trip
  - **Property 2: Polar Coordinate Round Trip**
  - **Validates: Requirements AC-7.2.1, AC-7.2.2, AC-7.2.3, AC-7.2.5**

- [x] 3.5 Write unit tests for PolarConverter
  - Test origin (0, 0) → (0, 0)
...
- [x] 4. Checkpoint - Ensure fusion and polar conversion tests pass
  - Ensure all tests pass, ask the user if questions arise.


- [ ] 5. Implement RingDetector class
  - [x] 5.1 Create `src/fusion/ring_detector.py` with RingDetector class
...
- [ ] 6. Implement SectorDetector class
  - [x] 6.1 Create `src/fusion/sector_detector.py` with SectorDetector class
...
- [ ] 7. Implement ScoreCalculator class
  - [x] 7.1 Create `src/fusion/score_calculator.py` with ScoreCalculator class
  ...
  - [x] 7.2 Implement score calculation logic
  ...
  - [x] 7.3 Implement event creation
...
- [x] 7.4 Write property test for score calculation
  - **Property 6: Score Calculation Correctness**
  - **Validates: Requirements AC-7.5.1, AC-7.5.2, AC-7.5.3**

- [x] 7.5 Write property test for event structure completeness
...
- [x] 7.6 Write unit tests for ScoreCalculator
...
- [x] 8. Checkpoint - Ensure all scoring tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [x] 9. Integration with main loop
  - [x] 9.1 Update main.py to initialize ScoreCalculator
    - Import ScoreCalculator from src.fusion
    - Initialize with config after coordinate mapper
    - _Requirements: AC-7.1.1_
  
  - [x] 9.2 Integrate fusion into dart detection workflow
  ...
  - [x] 9.3 Add logging for fusion results
  ...
  - [x] 9.4 Save DartHitEvent to JSON
...
- [x] 10. Add configuration to config.toml
...
- [x] 11. Create test data and fixtures
